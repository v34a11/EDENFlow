...
def local_correlation_softmax(
    ...
    """
    Optimized version for large-batch training.
    ...
    coords = coords_init.view(b, 2, -1).permute(0, 2, 1)    # [B,HW,2]
    ...
    sample_coords = coords.unsqueeze(-2) + window_grid      # [B,HW,K²,2]
    ...
    B, HW, K2, _ = sample_coords_norm.shape
    Q = HW * K2

    sample_coords_norm = sample_coords_norm.reshape(B, Q, 1, 2).contiguous()
    feature1 = feature1.contiguous()

    # ---------------------------------------------------
    # Chunked grid_sample
    # ---------------------------------------------------
    chunk_size = 8192   # tune: 4096 / 8192 / 16384
    outputs = []
    ...
    with torch.cuda.amp.autocast(enabled=True):
        for start in range(0, Q, chunk_size):
            ...
            grid_chunk = sample_coords_norm[:, start:end].contiguous()

            out_chunk = F.grid_sample(
                feature1,
                grid_chunk,
                mode='bilinear',
    ...
    window_feature = torch.cat(outputs, dim=2)              # [B,C,Q,1]
    window_feature = window_feature.reshape(B, c, HW, K2)
...
#     sample_coords_norm = sample_coords_norm.reshape(B, h * w * K2, 1, 2)
...
#     window_feature = window_feature.reshape(B, c, h * w, K2).permute(0, 2, 1, 3)

#     feature0_view = feature0.permute(0, 2, 3, 1).reshape(b, h * w, 1, c)
